...
sub test_load : Tests {
    is( Spectre::Reports->get_reports_count, 0, "no reports" );
    foreach my $archive_file ( glob("$load_dir/*.tar.gz") ) {
        Spectre::Report->new_from_tap_archive($archive_file);
    }
    is( Spectre::Reports->get_reports_count, 6, "6 reports" );

    my $reports =
      Spectre::Reports->get_reports( query => [ name => 'development-03-14-14_59_45-bufl' ] );
